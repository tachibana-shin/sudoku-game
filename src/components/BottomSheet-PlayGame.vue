<template>
  <v-bottom-sheet :value="value" @input="$emit('input', $event)">
    ...
    <v-list>
      ...
      <v-list-item v-for="tile in 5" :key="tile" @click="sheet = false; newGame(58)">
        <v-list-item-avatar>
          <v-img height="22px" width="22px" :src="require('@/assets/mode-speed.png')" />
        </v-list-item-avatar>
        <v-list-item-title> Nhanh </v-list-item-title>
      </v-list-item>
    </v-list>
  </v-bottom-sheet>
</template>
<script>
  ...
  function convertArray(sudoku) {
    ...
    return arr.map((x, i) => arr[i].map(y => ({ value: y, readOnly: true })))
  }
  ...
  export default {
    ...
    methods: {
      generateNewMapSudoku(level) {
        const mapOffset = []
        const mapSudoku = generate().map((item, y) => item.map((item, x) => {
          mapOffset.push({ x, y })
          return {
            ...item,
            notes: [],
        ...
        for (let index = 0; index < level; index++) {
          const offset = Math.round(Math.random() * (mapOffset.length - 1))
          const { x, y } = mapOffset.splice(offset, 1)[0]

          mapSudoku[y][x].readOnly = false
          mapSudoku[y][x].valueNow = null
        }

        return mapSudoku
      },
      newGame(level) {
        this.$store.commit("resetPlayground")
        this.$store.commit("setMapSudoku", this.generateNewMapSudoku(level))
